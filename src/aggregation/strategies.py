...
def create_aggregator(node_hash, agg_args: dict = {}):
    ...
    if aggregator_type == 'fedavg':
        ...
        return TrimmedMean(beta=agg_args.get("trimmed_mean_beta", 0.2))
...
class TrimmedMean(Aggregator):
    ...
    def aggregate(self, model_paths):
        ...
        num_excluded = self.beta * num_models
        model = torch.load(model_paths[0], map_location=device)
        ...
        for layer in inputs:
            ...
            largest, _ = torch.topk(inputs[layer], num_excluded, 0)
            neg_smallest, _ = torch.topk(-inputs[layer], num_excluded, 0)
            new_stacked = torch.cat([inputs[layer], -largest, neg_smallest]).sum(0)
            new_stacked /= num_models - 2 * num_excluded
